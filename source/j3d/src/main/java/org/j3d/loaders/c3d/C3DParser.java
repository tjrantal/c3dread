...
public class C3DParser
{
	...
	//event times from the parameter block
	private double[] eventTimes;
	
	//Event contexts from the parameter block
	private String[]  eventContexts;
	/** Event labels extracted from the parameter block */
    private String[] eventLabels;

    /** Event descriptions extracted from the parameter block */
	...
	public C3DEventData getEventParameter(){
		return new C3DEventData(eventLabels,eventDescriptions,eventContexts,eventTimes);
	}
    ...
    private boolean parseHeader() throws IOException
    {
		...
		try{
			...
			header.numAnalogChannels = 0;
			header.analogSampleRate = 0;
		}
     ...
     * @return true if the parsing should continue to the next section
     * @throws IOException some problem reading the basic file.
	 * Modified by Timo Rantalainen 19.05.2021
     */
    private boolean parseParams(boolean retainData) throws IOException
    {
        ...
        while(have_params)
        {
            ...
            if(id < 0)
            {
                ...
                if(name.equals("ANALOG")){
                    analog_group_id = id;
					//System.out.println(String.format("ANALOG group found!!! %d",id));
                }else{ 
					if(name.equals("POINT")){
						point_group_id = id;
						//System.out.println(String.format("POINT group found!!! %d",id));
					}else{
						if(name.equals("EVENT")){
							event_group_id = id;
							//System.out.println(String.format("EVENT group found!!! %d",id));
						}
					}
				}
            ...
            {
				if (id == event_group_id){
					//System.out.println("Found EVENT parameter");
				}
		...
		if (event_group_id != 0){
			C3DParameterGroup grp = (C3DParameterGroup)param_groups.get(event_group_id);
			
			
			//System.out.println(grp.toString());
			
			eventLabels = null; //new String[header.numTrajectories];
            eventDescriptions = null; //new String[header.numTrajectories];
			eventContexts = null;
			eventTimes = null;
			
			//TIMES
			C3DFloatParameter times_param =
                (C3DFloatParameter)grp.getParameter("TIMES");
				
			if(times_param == null)
                errorReporter.errorReport(LABEL_PARAM_MISSING_ERR, null);
            else
            {
                float[][] times = (float[][])times_param.getValue();
				//System.out.println(String.format("times size %d %d",times.length,times[0].length));
				eventTimes = new double[times.length];
                for (int e = 0;e<times.length;++e){
					//System.out.println(String.format("times %d %.03f %.03f",e,times[e][0],times[e][1]));
					eventTimes[e] = times[e][0]*60d+times[e][1];
				}
            }
			...
			//CONTEXTS
			C3DStringParameter context_param =
                (C3DStringParameter)grp.getParameter("CONTEXTS");

            if(context_param == null)
                errorReporter.errorReport(LABEL_PARAM_MISSING_ERR, null);
            else
            {
                String[] contexts = (String[])context_param.getValue();
				eventContexts = new String[contexts.length];
                System.arraycopy(contexts,
                                 0,
                                 eventContexts,
                                 0,
                                 contexts.length);
            ...
            else
            {
				...
				eventLabels = new String[labels.length];
            ...
            else
            {
                String[] descriptions = (String[])desc_param.getValue();
				 eventDescriptions = new String[descriptions.length];
			...
			if (params != null){
				for (int p = 0;p<params.length;++p){
					...
					System.out.println(params[p].toString());
						...
						System.out.println("params[p].toString() "+e.toString());
					}
				}
			}else{
				System.out.println("NULL grp.getParameters()");
			}
			*/
